...
def format_price_display(p):
    ...
    # Använd 4 decimaler för små priser (under 10), annars 2.
    price_format = f"{p:,.4f}" if p < 10 else f"{p:,.2f}"
    # Formatering för att använda mellanslag som tusentalsavgränsare och komma som decimalavgränsare (svensk standard)
...
# MODIFIERAD FUNKTION: check_and_send_alerts
def check_and_send_alerts(alert_data, r_instance):
    """
    Kontrollerar alla kryptovalutors procentuella förändringar.
    alert_data är nu en dict som innehåller alla förändringar och priser.
    ...
    for coin_symbol, data in alert_data.items():
        ...
        # Formatera priset i EUR (tar bort tusentalsavgränsare för att göra det enkelt för Markdown)
        formatted_price = f"{current_price_eur:,.4f}".replace(",", "TEMP").replace(".", ",").replace("TEMP", "") 
...
def background_data_fetch(redis_instance):
    ...
    while True:
        ...
        try:
            ...
            all_percent_changes = {}
            all_24h_range_ohlc = {} 
            alert_data_for_sending = {} # NY DATASTRUKTUR för larm
            ...
            for label, ticker in CRYPTO_PAIRS.items():
                ...
                # Lägg till data i den nya alert-strukturen
                alert_data_for_sending[coin_symbol] = {
                    'changes': percent_changes,
                    'price_eur': current_price_eur # NU INGÅR PRIS
                }
                
                time.sleep(0.1) 
            
            # --- Steg 2: KÖR ALERT KONTROLL (skickar alert_data_for_sending) ---
            if redis_instance:
...
def update_trendline_visibility(chart_data_store, currency, selected_trends, coin_symbol): # (oförändrad)
